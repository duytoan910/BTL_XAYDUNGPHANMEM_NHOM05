...
{
    public partial class frmDeleteLateCharge : Form
    {
        CustomerBLL bCus;
        LateChargeBLL bCharge;
        double sumMoney = 0;

        public frmDeleteLateCharge()
        {
            InitializeComponent();
        }
   
        private void frmDeleteLateCharge_Load(object sender, EventArgs e)
        {
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Xác nhận xóa phí trễ hạn ?", "Xóa phí", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                //Xóa những phí đã chọn
                foreach (DataGridViewRow row in dgvLateCharge.Rows)
                {
                    ...
                    {
                        bCharge.deleteLateCharge(id);
                    }
                }
                MessageBox.Show("Hoàn tất xóa phí !", "Xóa phí", MessageBoxButtons.OK, MessageBoxIcon.Information);
